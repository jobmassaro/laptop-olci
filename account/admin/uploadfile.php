    			...
    			foreach($rowData[0] as $k=>$v)
    			{
    				...
    				if($row >= $start)
    				{
    					...
    					if($row != $tmp)
    					{
    						...
    						date_add($date, date_interval_create_from_date_string('' .$irw[22] .' days'));
    						...
    						$sql = "INSERT INTO tbl_olci_body_" . $idvalue ." (id, id_head,prog,p0,p1,p2,p3,p4,p5,p6,STAZIONE,CODICE_STRUTTURA,DISEGNO_FORNITORE,DISEGNO_CLIENTE,FILE_ORIGINALE," ."TOTALE_FOGLI,FOGLI_RICEVUTI,DESCRIZIONE_ITALIANO,DESCRIZIONE_INGLESE,DESCRIZIONE_LINGUA_LOCALE," .
    							...
    							",'".$irw[5]."','".$irw[6]."','".$irw[7]."','".$irw[8]."','".$irw[9]."','" . $irw[10]. "','" . $irw[11] . "','" .$irw[12] ."','".$irw[13]."','".$irw[14]."','".$irw[15]."','".$irw[16]."','".$irw[17]."','".$irw[18]."','".$irw[19]."','".$irw[20]."','".$irw[21]."','". date_format($date, 'd/m/Y') ."','". $irw[23] ."')";  
...
/*
							...
							/*	var_dump('idvalue => ' .$idvalue);
								...
								$date = date_create('30-12-1899');
    							date_add($date, date_interval_create_from_date_string('' .$irw[22] .' days'));
								var_dump('Data =>' . date_format($date, 'Y-m-d'));
