...
$sql = "INSERT INTO tbl_olci_lista_distinta_base(id_distbase, disegno_no) VALUES(" . $id_distbase. ",'". mysqli_real_escape_string($con, $data->numdis) ."')";
...
if(isset($_FILES['UploadFileField']))
{
		...
		}else
		{
    				...
    				if($row >= $start)
    				{
    					...
    					if($row >= 6 && $k <=15)
    					{
    						if($insert == false)
    						{
                                ...
                                $sql = "UPDATE tbl_olci_lista_distinta_base SET gruppo='".$code."',data='" . date_format($ddate, 'd/m/Y') ."',titolo='". $titolo . "' WHERE disegno_no='". $params ."' AND id=" . $num ." AND id_distbase=" . $relazbase;
            ...
            if(count($irw)>0)
            {
                  $sql = "CREATE TABLE  IF NOT EXISTS tbl_olci_distinta_base_" . $num ." 
                    (
			...
			for($j= 6; $j < count($irw); $j++ )
			{
				if($irw[$j][1] != null)
				{
					...
					//echo $irw[$j][1];
					//echo '<br />';
						$sql ="INSERT INTO tbl_olci_distinta_base_". $num ."(
								`id_distbase`,
								`gruppo`,
								`data`,
								`titolo`,
								...
								`consegne`)
								VALUES(
									". $relazbase.",'".$code."','" . $data ."','". $titolo . "'," .$irw[$j][1] .",'" .$irw[$j][2]."',".$irw[$j][3] .",'" .$irw[$j][4] ."','". $irw[$j][5]."','" .$irw[$j][6] ."','" .$irw[$j][7] ."','" .$irw[$j][8] ."','" . $irw[$j][9] ."','" .$irw[$j][10] ."','" . $irw[$j][11]. "','" .$irw[$j][12]."')";
			//	echo '<br/>';
			//	echo $sql;
									mysqli_query($connect, $sql);
/*
